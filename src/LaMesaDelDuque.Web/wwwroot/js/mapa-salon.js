...
(function () {
    ...
    function actualizarTimers() {
        document.querySelectorAll(MESA_SELECTOR).forEach(mesa => {
            ...
            if (mesa.dataset.mesaEstado !== 'Ocupada' || !ocupadaDesde || !timerEl) {
    ...
    function limpiarClasesEstado(mesa) {
        mesa.classList.remove(
            'lmd-mapa--disponible',
            'lmd-mapa--ocupada-verde',
            'lmd-mapa--ocupada-amarillo',
            'lmd-mapa--ocupada-rojo',
            'lmd-mapa--reservada',
            'lmd-mapa--mantenimiento',
            'lmd-mapa--inactiva',
            'lmd-mapa--neutral'
        );
    }

    function claseEstadoMesa(estado) {
        return {
    ...
    function aplicarEstadoMesa(mesa, estado, ocupadaDesde) {
        if (!mesa) return;
        mesa.dataset.mesaEstado = estado;
        limpiarClasesEstado(mesa);
        mesa.classList.add(claseEstadoMesa(estado));

        if (estado === 'Ocupada') {
            mesa.dataset.mesaOcupadaDesde = ocupadaDesde || mesa.dataset.mesaOcupadaDesde || new Date().toISOString();
        } else {
        ...
        if (mesa.dataset.mesaId === mesaActivaId && PANEL.style.display !== 'none') {
            PANEL_ESTADO.textContent = estado;
            PANEL_ESTADO.className = 'lmd-mapa-panel-estado ' + claseEstadoMesa(estado);
            PANEL_TIMER.textContent = '';
        }
    ...
    function abrirPanel(mesaEl) {
        ...
        const puedeCambiarEstado = mesaEl.dataset.puedeCambiarEstado === 'true' || puedeEditar;
        ...
        PANEL_ESTADO.classList.add(claseEstadoMesa(estado));
        ...
        // Disable buttons for current estado. Mesero puede cambiar estado,
        // pero solo Administrador/Encargado pueden arrastrar posiciones.
        PANEL_ACCIONES.querySelectorAll('.lmd-mapa-btn').forEach(btn => {
            btn.disabled = btn.dataset.estado === estado || !puedeCambiarEstado || !activa;
        });
    ...
    PANEL_ACCIONES.querySelectorAll('.lmd-mapa-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (data.exito) {
                    const mesa = document.querySelector(`${MESA_SELECTOR}[data-mesa-id="${mesaActivaId}"]`);
                    aplicarEstadoMesa(mesa, nuevoEstado);
                    cerrarPanel();
                } else {
    ...
    async function iniciarSignalR() {
        ...
        signalRConnection.on('MesaActualizada', (payload) => {
            const mesa = document.querySelector(`${MESA_SELECTOR}[data-mesa-id="${payload.mesaId}"]`);
            if (!mesa) return;
            aplicarEstadoMesa(mesa, payload.estado);
        });
    ...
    function sincronizarMesas(mesas) {
        ...
        mesas.forEach(m => {
            ...
            aplicarEstadoMesa(mesa, estado, m.ocupadaDesde);
        });
        actualizarTimers();
    }
