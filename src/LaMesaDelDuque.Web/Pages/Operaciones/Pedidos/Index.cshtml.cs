...
public class IndexModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostCrearJsonAsync()
    {
        ...
        try
        {
            ...
            return new JsonResult(new { ok = true, pedidoId = pedido.Id, total = pedido.Total, estado = pedido.Estado, detalles = detallesResponse });
        }
        catch (ReglaDominioException ex)
        {
    ...
    public async Task<IActionResult> OnPostEnviarMasJsonAsync()
    {
        ...
        try
        {
            ...
            return new JsonResult(new { ok = true, total = pedido?.Total ?? 0, estado = pedido?.Estado, detalles = detallesResponse });
        }
