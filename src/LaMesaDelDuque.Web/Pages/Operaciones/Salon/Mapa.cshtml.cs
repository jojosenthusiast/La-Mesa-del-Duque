...
public class MapaModel : PageModel
{
    ...
    public MapaModel(
        ...
        IZonasSalonServicio zonasServicio,
        IHubContext<PedidosHub> hubContext,
        ILogger<MapaModel> logger)
    {
    ...
    public async Task<IActionResult> OnPostActualizarPosicionAsync([FromBody] ActualizarPosicionRequest request)
    {
        ...
        try
        {
            ...
            await _hubContext.Clients.All.SendAsync("MesaMovida", new
            {
                mesaId = request.MesaId,
                posX = request.PosicionX,
                posY = request.PosicionY
            });
    ...
    public async Task<IActionResult> OnPostCambiarEstadoAsync([FromBody] CambiarEstadoRequest request)
    {
        if (User?.IsInRole("Administrador") != true
            && User?.IsInRole("Encargado") != true
            && User?.IsInRole("Mesero") != true)
        {
            return new JsonResult(new { exito = false, error = "No autorizado." }) { StatusCode = 403 };
        }
        ...
        try
        {
            ...
            await _hubContext.Clients.All.SendAsync("MesaActualizada", new
            {
                mesaId = request.MesaId,
